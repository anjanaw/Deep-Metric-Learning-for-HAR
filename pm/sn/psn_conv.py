...
from keras.layers import Dense, Dropout, Input, Lambda, Conv1D, MaxPooling1D, Flatten
...
for test_id in test_ids:
    ...
    for x in range(epochs):
        digit_indices = [np.where(_train_labels == i)[0] for i in range(len(read.activity_list))]
        x_pairs, y_pairs = create_pairs(_train_data, digit_indices, len(read.activity_list))
    ...
    result = 'prototype_sm_conv, 3nn,' + str(test_id) + ',' + str(acc)
    print(result)
    read.write_data('sn_conv.csv', result)
